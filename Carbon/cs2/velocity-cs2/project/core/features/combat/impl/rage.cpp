...
namespace features::combat {
	...
	void rage::run_gun( systems::input::usercmd* cmd, const aim_context& ctx, const systems::local::snapshot& local, bool allow_fire, std::vector<candidate>* pre_candidates )
	{
		...
		if ( !shot_viable && best.valid && ctx.on_ground && config.early_counterstrafe_predict.value )
		{
		...
		if ( shared_ctx.weapon_type == cstypes::weapon_type::sniper && !ctx.is_scoped && config.auto_scope.value )
		{
			if ( best.valid || this->m_should_stop || !current_hits.empty( ) )
			{
	...
	{
		std::vector<scan_hit> all_hits;
		all_hits.reserve( candidates.size( ) * 16 );
	...
	{
		if ( !cand.pawn || cand.record_count <= 0 || cand.health <= 0 || !record || !record->valid )
			return {};
		...
		const auto flags = memory::read<std::uint32_t>( cand.pawn + SCHEMA( "C_BaseEntity", "m_fFlags"_hash ) );
		const auto target_in_air = ( flags & cstypes::entity_flags::on_ground ) == 0;
		const auto force_body = config.body_aim.value || ( target_in_air && config.baim_air.value );
		...
		for ( auto idx = 0; idx < scan_count; ++idx )
		{
			...
			if ( !hb || hb->bone < 0 || hb->bone >= 128 || ( record->bone_count > 0 && hb->bone >= record->bone_count ) )
			{
				continue;
			}
	...
	rage::target rage::select_best( const aim_context& aim_ctx, const std::vector<scan_hit>& hits, float eval_inaccuracy ) const
	{
		...
		constexpr auto top_k_per_record{ 16 };

		const auto& config = settings::g_combat.m_ragebot.get_group( g_shared.ctx( ).weapon_type );

		const auto head_low_hp_bonus = [ & ]( const scan_hit& h ) -> float
			{
				if ( !config.head_priority_low_hp.value || h.hitbox_index != 0 || h.health > config.head_priority_hp.value )
				{
					return 0.0f;
				}
	...
	{
		const auto can_kill = h.damage >= static_cast< float >( h.health );
		const auto is_body = h.hitbox_index >= 1 && h.hitbox_index <= 6;
		const auto lethal_baim_bonus = ( config.baim_lethal.value && can_kill && is_body ) ? 50000.0f : 0.0f;
		const auto lethal_bonus = can_kill ? 100000.0f : 0.0f;
		const auto direct_bonus = h.penetrated ? 0.0f : 5000.0f;
		const auto center_bonus = h.is_center ? 2000.0f : 0.0f;

		return lethal_bonus + lethal_baim_bonus + direct_bonus + center_bonus + h.damage * 20.0f +
			static_cast< float >( hitgroup_priority( h.hitbox_index ) ) * 10.0f - h.fov + head_low_hp_bonus( h );
	};
		...
		std::unordered_map<std::uintptr_t, float> pawn_speeds;

		// Target stickiness: with many valid targets the score often near-ties
		// and the pick flips every tick ("goes wild"). Bias toward the enemy we
		// already committed to so it only switches on a real score gain.
		static std::uintptr_t s_last_target_pawn{ 0 };

		const auto get_pawn_speed = [ & ]( std::uintptr_t pawn ) -> float
			{
				const auto it = pawn_speeds.find( pawn );
				if ( it != pawn_speeds.end( ) )
				{
					return it->second;
				}

				const auto speed = memory::read<math::vector3>( pawn + SCHEMA( "C_BaseEntity", "m_vecVelocity"_hash ) ).length_2d( );
				pawn_speeds.emplace( pawn, speed );
				return speed;
			};

		const auto base_needed_hc = config.hitchance_override.value
		...
		for ( auto& group : groups )
		{
			...
			for ( const auto idx : group.hit_indices )
			{
				...
				if ( h.bone_index < 0 || h.bone_index >= 128 || ( group.record->bone_count > 0 && h.bone_index >= group.record->bone_count ) )
				{
					continue;
				}

				auto needed_hc = base_needed_hc;
				if ( config.adaptive_hitchance.value && get_pawn_speed( h.pawn ) < 5.0f )
				{
					const auto reduction = static_cast< float >( config.adaptive_hitchance_reduction ) / 100.0f;
				...
				if ( can_kill )
				{
					...
					if ( config.baim_lethal.value && ( h.hitbox_index >= 1 && h.hitbox_index <= 6 ) )
					{
						score += 250000.0f;
					}
				}
...
score += h.penetrated ? 0.0f : 250.0f;
				...
				score += head_low_hp_bonus( h );
				score -= h.fov * 0.1f;
				score += ( h.pawn == s_last_target_pawn ) ? 3000.0f : 0.0f;

				// Aim at the pose the server actually presents. gather_candidates
				// offers the record nearest the interpolation rewind specifically
				// because shooting only the newest record left the target's moving
				// or desynced pose drifting from the rewind, registering as clean
				// misses. Score the rewind-matched record first so the transmitted
				// shot lines up with the pose hit registration will use; lethal and
				// large damage advantages still outrank it.
				const auto rewind_tick = g_shared.ctx( ).current_tick - g_shared.sh( ).lerp_ticks_int( );
				const auto rewind_dt = std::abs( h.record->tick - rewind_tick );
				if ( rewind_dt <= 0 )
				{
					score += 50000.0f;
				}
				...
				evaluated.push_back( evaluated_hit{ idx, hc, needed_hc, score } );
			}
		}
		...
		for ( const auto& e : evaluated )
		{
			...
			if ( is_better )
			{
				...
				best.needed_hitchance = e.needed_hitchance;
				best.score = e.score;
				best.valid = true;
			}
		}
		...
		if ( best.valid )
		{
			s_last_target_pawn = best.hit.pawn;
		}
	...
	{
		if ( !hit.record || !hit.record->valid || hit.bone_index < 0 || hit.bone_index >= 128 || ( hit.record->bone_count > 0 && hit.bone_index >= hit.record->bone_count ) )
		{
			return 0.0f;
		}

		const auto geometric = g_shared.calculate_hitchance( hit.source_eye.position, hit.aim_angle, hit.hitbox, hit.record->bones[ hit.bone_index ], inaccuracy, ctx.spread );

		// The geometric Monte-Carlo assumes a vacuum cone. The server only counts
		// bullets that physically reach the hitbox, so fold the occlusion ratio of
		// the real spread cone into the gate. Rays clipped by a wall, roof or an
		// ally before the target are misses the geometric number silently counted
		// as hits, which is how the reticle read 95% while the spray ate the wall.
		const auto distance_to_target = ( hit.position - hit.source_eye.position ).length( );
		if ( distance_to_target <= 0.0f )
		{
			return geometric;
		}

		const auto occlusion = g_shared.measure_spread_occlusion( hit.source_eye.position, hit.aim_angle, inaccuracy, ctx.spread, hit.pawn, distance_to_target );
		return geometric * occlusion;
	}
	...
	float rage::get_standing_inaccuracy( const systems::local::snapshot& local, const aim_context& ctx ) const
	{
		...
		const auto current_inacc = g_shared.get_inaccuracy( false );
		const auto vel_inacc = g_shared.get_inaccuracy_at_velocity( local.pawn, velocity );
		const auto accurate_vel_inacc = ( vel_inacc > 0.0f && vel_inacc < 0.9f ) ? vel_inacc : current_inacc;

		return std::max( current_inacc, accurate_vel_inacc );
	}
		...
		for ( auto& cand : candidates )
		{
			for ( auto ri = 0; ri < cand.record_count; ++ri )
			{
				...
				for ( auto i = 0; i < hitbox_set.count; ++i )
				{
					...
					if ( hb.bone < 0 || hb.bone >= 128 || ( record->bone_count > 0 && hb.bone >= record->bone_count ) )
					{
						continue;
					}
	...
	void rage::fire_gun( systems::input::usercmd* cmd, const target& tgt, bool was_forced, const math::vector3& shoot_eye, const systems::local::snapshot& local, bool subtick_attack, bool aim_only )
	{
		...
		// First-bullet accuracy: the opening shot of an engagement fires while
		// autostop is still decelerating, so its inaccuracy is the largest of the
		// fight and an uncorrected bullet scatters far off ("shoots into a random
		// place / misses with the first bullet"). Mirror the no-spread handler:
		// whenever the CURRENT inaccuracy is material, cancel the seeded random
		// component so the first bullet is as reliable as every follow-up.
		auto current_inaccuracy = g_shared.get_inaccuracy( false );

		// Airborne shots: the engine's per-frame accuracy read under-reports the
		// cone while the player is rising/falling, which is why build_context
		// floors the hitchance estimate with the analytic jump inaccuracy. Apply
		// the same floor to the no-spread correction: cancelling against the raw
		// (too small) read leaves the difference of the two cones, which shows up
		// as the "slight left/right miss around the head" on airborne sniper shots.
		if ( !( systems::g_prediction.pre( ).flags & cstypes::entity_flags::on_ground ) && shared_ctx.weapon_vdata )
		{
			const auto jump_initial = memory::read<float>( shared_ctx.weapon_vdata + SCHEMA( "CCSWeaponBaseVData", "m_flInaccuracyJumpInitial"_hash ) );
			const auto jump_apex = memory::read<float>( shared_ctx.weapon_vdata + SCHEMA( "CCSWeaponBaseVData", "m_flInaccuracyJumpApex"_hash ) );
			current_inaccuracy = std::max( current_inaccuracy,
				g_shared.get_air_inaccuracy( systems::g_prediction.pre( ).velocity.z, jump_initial, jump_apex ) );
		}

		const bool correct_spread = config.no_spread.value;

		auto aim_angle = tgt.hit.aim_angle;
		math::helpers::normalize_angles( aim_angle );

		auto write_angle = math::vector3{ aim_angle.x - aim_punch.x, aim_angle.y - aim_punch.y, 0.0f };
		math::helpers::normalize_angles( write_angle );

		if ( correct_spread )
		{
			g_shared.ctx( ).inaccuracy = current_inaccuracy;
			const auto& w = g_shared.ctx( );

			// Keep the un-corrected view (what we truly want to hit) as the
			// residual reference; the transmitted view becomes the solver's
			// fixed point.
			const auto target_angle = write_angle;
			...
			math::vector3 target_dir{}, target_left{}, target_up{};
			math::helpers::angle_vectors_left( target_angle, &target_dir, &target_left, &target_up );

			const auto probe = [ & ]( const math::vector3& ang )
			{
				const auto seed = g_shared.get_spread_seed( ang, stamp_tick );
				const auto dev = g_shared.calculate_spread( seed, w.inaccuracy, w.spread, w.recoil_index, w.item_def_idx, w.num_bullets );

				math::vector3 f{}, l{}, u{};
				math::helpers::angle_vectors_left( ang, &f, &l, &u );
				return ( f + l * dev.x + u * dev.y ).normalized( );
			};

			const auto residual = [ & ]( const math::vector3& dir )
			{
				return std::acos( std::clamp( target_dir.dot( dir ), -1.0f, 1.0f ) );
			};

			const auto raw_res_deg = math::helpers::rad_to_deg( residual( probe( target_angle ) ) );

			const auto corrected = g_shared.find_spread_correction( target_angle, stamp_tick );
			write_angle = corrected;
			math::helpers::normalize_angles( write_angle );

			const auto res_deg = math::helpers::rad_to_deg( residual( probe( write_angle ) ) );

			// Refuse only the degenerate class: a shot the solver could not
			// improve (still sits at the raw aim) with a material residual is the
			// full uncancelled cone - the scatter seen on fast-firing guns in the
			// first frames of an engagement. A resolved fixed point may be mildly
			// imprecise (scoped weapon seeds resolve on a coarser lattice, so
			// bolt-action rifles legitimately sit above the spray guns' floor)
			// and must keep firing. An absurd residual is incoherent state.
			const auto solved_well = ( raw_res_deg - res_deg ) > 0.05f || res_deg < 0.15f;
			const auto garbage = !solved_well && res_deg > 0.35f;

			if ( settings::g_misc.m_impacts.console_log.value )
			{
				logging::console::print(
					xs( "[nospread] stamp {} base {} inacc {:.4f} spread {:.4f} res {:.3f} deg (raw {:.3f}){}" ),
					stamp_tick, tick_base, w.inaccuracy, w.spread, res_deg, raw_res_deg,
					!solved_well ? xs( " UNSOLVED" ) : "" );
			}

			if ( garbage || res_deg > 2.0f )
			{
				return;
			}
		}
		...
		if ( aim_only )
		{
			// Lethal-charge hold: queue the corrected aim so whatever the server
			// interpolates that tick is staring at the target, but never emit an
			// attack, mark a shot, or record anything. The burst goes out on the
			// tick the kill actually flips.
			if ( const auto angles = base->mutable_viewangles( ) )
			{
				angles->set_x( write_angle.x );
				angles->set_y( write_angle.y );

				if ( config.no_spread.value )
				{
					angles->set_z( write_angle.z );
				}
			}
		...
		if ( tgt.hit.pawn && tgt.hit.record && tgt.hit.record->valid && tgt.hit.record->bone_count > 0 )
		{
		...
		rendering::s_last_shot_time = GetTickCount64( );
		const auto record_time = tgt.hit.record ? cstypes::tick_fraction::from_value( tgt.hit.record->simulation_time / cstypes::tick_interval ) : cstypes::tick_fraction{};
		...
		for ( auto i = 0; i < history_size; ++i )
		{
			...
			if ( auto interp = entry->mutable_sv_interp0( ) )
			{
			...
			if ( auto interp = entry->mutable_sv_interp1( ) )
			{
				interp->set_src_tick( -1 );
				interp->set_dst_tick( -1 );
				interp->set_frac( 0.0f );
			...
			if ( auto interp = entry->mutable_cl_interp( ) )
			{
				interp->set_frac( 0.0f );
			}
		}
		...
		// Hide-shot hiding must judge "facing away" from the USER's real view,
		// not from the command base. With AA on, prediction leaks the previous
		// tick's fake into the eye angles that seed this command's composition,
		// so base->viewangles() holds the fake - which deliberately points AWAY
		// from enemies - and facing_away would evaluate true on nearly every
		// subtick attack. The burst base then gets reversed 180 degrees and the
		// first pulse goes into the wall ("rage barely hits when AA is on").
		math::vector3 forward{};
		bool view_ok{ false };
		const auto real_view = systems::g_input.get_view_angles( &view_ok );
		if ( view_ok )
		{
			math::helpers::angle_vectors_left( real_view, &forward );
		}
		else if ( const auto angles = base->viewangles( ) )
		{
	...
	std::vector<math::vector3> rage::generate_multipoints( const systems::hitboxes::entry& hitbox, const math::vector3& center, const math::quaternion& bone_rot, float pointscale, const math::vector3& shoot_pos, float inaccuracy ) const
	{
		...
		constexpr auto k_dynamic_pointscale_bias = 0.9f;
