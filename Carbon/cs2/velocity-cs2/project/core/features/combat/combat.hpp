...
namespace features::combat {
	...
	public:
		...
		{
		public:
			void on_record( lagcomp::record& rec );

		private:
			void rotate_head_chain( lagcomp::record& rec, float yaw_delta_deg ) const;
		};
		...
		// Resolved weapon cycle time (s), falling back to a per-weapon-type
		// cadence when the schema read misreads so the fire-rate gate can
		// never be silently disabled.
		[[nodiscard]] float cycle_time( ) const;

		// Same-target re-fire hold: note a fired shot at a pawn, a confirmed
		// hit (clears the hold so reliable targets keep the trigger), or a
		// resolved miss (backs off re-engaging that pawn briefly).
		void note_fired( std::uintptr_t pawn, float now );
		void note_hit( std::uintptr_t pawn );
		void note_miss( std::uintptr_t pawn, float now );
		[[nodiscard]] bool target_held( std::uintptr_t pawn, float now ) const;
		...
		[[nodiscard]] float measure_spread_occlusion( const math::vector3& shoot_position, const math::vector3& aim_angle, float inaccuracy, float spread, std::uintptr_t target_pawn, float target_distance, int samples = 16 ) const;
	...
	private:
		...
		std::atomic<std::uintptr_t> m_hold_pawn{ 0 };
		std::atomic<float> m_hold_until{ 0.0f };
	};
	...
	private:
		...
		private:
			...
			[[nodiscard]] bool is_fake_pitch( float pitch ) const;
			void track_pitch( float& pitch );

			static constexpr int k_lby_breaker_period{ 70 }; // ~1.1 s @ 64 tick
			static constexpr int k_lby_breaker_hold{ 1 };

			math::vector3 m_old_angles{};
			math::vector3 m_modified_angles{};
			math::vector3 m_last_real_angles{};
			bool m_last_real_valid{};

			// Last real pitch that was demonstrably NOT the transmitted fake:
			// fixed pitch modes (down/up) only ever send ±89, and prediction
			// occasionally reseeds the engine input view from the faked pawn eye
			// angles, so the "real" read can arrive on that boundary and pull the
			// camera to face straight up/down. This is the restore value (yaw has
			// no equivalent clean reference since fixed yaw vs real yaw share the
			// 0..360 circle, which is why its guards compare deltas instead).
			float m_last_real_pitch{};
			bool m_last_real_pitch_valid{};

			int m_yaw_side{};
			int m_jitter_flip{};
			int m_lby_elapsed_ticks{};
			int m_lby_break_ticks{};
			bool m_lby_break_now{};
			...
			// fake tracks the mouse instantly while threat/backstab steering
			// eases in instead of snapping (the per-tick re-target was what made
		...
		private:
			[[nodiscard]] int resolve_amount( const math::vector3& velocity ) const;

			int m_hold_counter{};
			bool m_has_held_angles{};
			math::vector3 m_held_angles{};
		};
	...
	private:
		...
		void run_gun( systems::input::usercmd* cmd, const aim_context& ctx, const systems::local::snapshot& local, bool allow_fire = true, std::vector<candidate>* pre_candidates = nullptr );
		...
		void fire_gun( systems::input::usercmd* cmd, const target& tgt, bool was_forced, const math::vector3& shoot_eye, const systems::local::snapshot& local, bool subtick_attack, bool aim_only = false );
		...
		enum class doubletap_state : int
		{
			passthrough = 0,
			burst = 1,
			hold = 2
		...
		[[nodiscard]] doubletap_state process_doubletap( systems::input::usercmd* cmd, const systems::local::snapshot& local, bool fire_requested, bool gate_on_lethal, bool lethal );
		void clear_attack( systems::input::usercmd* cmd );
	...
	private:
		...
		void draw_fov( xdraw::draw_list& draw_list, const math::vector3& view_angles, const math::vector3& aim_punch, const settings::combat::legitbot::weapon_group& config, bool rcs_active ) const;
		void draw_target_indicator( xdraw::draw_list& draw_list, const target_result& target, const config::col& color, bool is_locked ) const;
		...
		mutable std::uintptr_t m_last_target_pawn{ 0 };
		mutable int m_last_target_bone{ -1 };
		mutable math::vector3 m_stabilized_target_pos{};
		mutable bool m_has_stabilized_pos{ false };
		std::uint64_t m_current_target_since{ 0 }; // GetTickCount64() when current target was locked
		bool m_auto_pistol_cycle{ false };
		mutable std::uint64_t m_last_kill_time{ 0 };
	};
