...
namespace features::esp::projectile {
	...
	void overlay::on_render( xdraw::draw_list& draw_list, xdraw::draw_list& middle_draw_list )
	{
		...
		if ( overlay_cfg.m_smoke_timer.enabled.value )
		{
			this->render_smoke_timers( draw_list, middle_draw_list, overlay_cfg.m_smoke_timer );
		}
	...
	void overlay::add_indicator( xdraw::draw_list& draw_list, float cx, float cy, float dir_angle, bool has_arrow, float timer_frac, float alpha, bool is_fire, const settings::esp::projectile::overlay::indicator::group& cfg )
	{
		...
		{
			const xdraw::color bg_col = cfg.background_color.value;
			const auto ba = static_cast< std::uint8_t >( static_cast< float >( bg_col.a ) * alpha );
			draw_list.circle_filled( cx, cy, detail::icon_radius, xdraw::color{ bg_col.r, bg_col.g, bg_col.b, ba }, detail::segments );
		}

		const xdraw::color icon_col = cfg.icon_color.value;
		...
		if ( arc_end > 0.01f )
		{
			const xdraw::color arc_col = cfg.arc_color.value;
	...
	void overlay::add_inferno( xdraw::draw_list& draw_list, xdraw::draw_list& middle_draw_list, const systems::entities::cached& entity, const settings::esp::projectile::overlay::infernos& cfg )
	{
		...
		const auto delta_time = xdraw::delta_time( );
		const auto fire_count_offset = SCHEMA( "C_Inferno", "m_fireCount"_hash );
		const auto fire_pos_offset = SCHEMA( "C_Inferno", "m_firePositions"_hash );
		const auto fire_burning_offset = SCHEMA( "C_Inferno", "m_bFireIsBurning"_hash );

		const auto off_count = fire_count_offset ? fire_count_offset : 0x1960;
		const auto off_pos = fire_pos_offset ? fire_pos_offset : 0x1020;
		const auto off_burning = fire_burning_offset ? fire_burning_offset : 0x1620;

		auto count = memory::read<int>( entity.ptr + off_count );
		if ( count <= 0 || count > 64 )
		{
			count = memory::read<int>( entity.ptr + 0x8580 );
		}
		...
		{
			if ( !memory::read<bool>( entity.ptr + off_burning + i ) )
			{
				continue;
			}

			const auto position = memory::read<math::vector3>( entity.ptr + off_pos + i * sizeof( math::vector3 ) );
			...
			constexpr auto node_radius{ 60.0f };
			...
			for ( auto d = 0; d < num_directions; ++d )
			{
				...
				if ( state.current_radii[ idx ] < 1.0f )
				{
					state.current_radii[ idx ] = node_radius;
				}
				else
				{
					state.current_radii[ idx ] += ( node_radius - state.current_radii[ idx ] ) * lerp_t;
				}
	...
	void overlay::update_smoke( const systems::entities::cached& entity )
	{
		const bool is_blooming = memory::read<bool>( entity.ptr + SCHEMA( "C_SmokeGrenadeProjectile", "m_bDidSmokeEffect"_hash ) ) ||
			( memory::read<int>( entity.ptr + SCHEMA( "C_SmokeGrenadeProjectile", "m_nSmokeEffectTickBegin"_hash ) ) > 0 );
		...
		auto& state = this->m_smoke_states[ entity.ptr ];
		if ( !state.active )
		{
			state.spawn_time = std::chrono::steady_clock::now( );
			state.active = true;
			...
			const auto det_pos = memory::read<math::vector3>( entity.ptr + SCHEMA( "C_SmokeGrenadeProjectile", "m_vSmokeDetonationPos"_hash ) );
			if ( std::isfinite( det_pos.x ) && det_pos.length_sqr( ) > 1.0f )
			{
				state.origin = det_pos;
			}
			else
			{
				const auto node = memory::read<std::uintptr_t>( entity.ptr + SCHEMA( "C_BaseEntity", "m_pGameSceneNode"_hash ) );
				if ( node )
				{
					state.origin = memory::read<math::vector3>( node + SCHEMA( "CGameSceneNode", "m_vecAbsOrigin"_hash ) );
				}
			}
		}
	}

	void overlay::render_smoke_timers( xdraw::draw_list& draw_list, xdraw::draw_list& middle_draw_list, const settings::esp::projectile::overlay::smoke_timer& cfg )
	{
		if ( this->m_smoke_states.empty( ) )
		{
			return;
		}

		constexpr float k_smoke_lifetime = 20.0f;
		const auto dt = xdraw::delta_time( );
		const auto now = std::chrono::steady_clock::now( );
		const auto m = systems::g_view.matrix( );

		auto get_w = [ &m ]( const math::vector3& p ) -> float
		{
			return m[ 3 ][ 0 ] * p.x + m[ 3 ][ 1 ] * p.y + m[ 3 ][ 2 ] * p.z + m[ 3 ][ 3 ];
		};

		// Collect all currently active blooming smoke grenade entities from the world
		std::unordered_set<std::uintptr_t> active_smoke_entities;
		for ( const auto& proj : systems::g_entities.get_by_type( systems::entities::type::projectile ) )
		{
			if ( proj.schema_hash == "C_SmokeGrenadeProjectile"_hash )
			{
				const bool did_smoke = memory::read<bool>( proj.ptr + SCHEMA( "C_SmokeGrenadeProjectile", "m_bDidSmokeEffect"_hash ) ) ||
					( memory::read<int>( proj.ptr + SCHEMA( "C_SmokeGrenadeProjectile", "m_nSmokeEffectTickBegin"_hash ) ) > 0 );
				if ( did_smoke )
				{
					active_smoke_entities.insert( proj.ptr );
				}
			}
		}
		...
		for ( auto it = this->m_smoke_states.begin( ); it != this->m_smoke_states.end( ); )
		{
			...
			const bool is_entity_still_smoking = active_smoke_entities.contains( it->first );
			if ( !is_entity_still_smoking )
			{
				state.active = false;
			}

			const auto elapsed = std::chrono::duration<float>( now - state.spawn_time ).count( );
			const auto remaining = ( !state.active ) ? 0.0f : std::max( 0.0f, k_smoke_lifetime - elapsed );
			const auto frac = std::clamp( remaining / k_smoke_lifetime, 0.0f, 1.0f );

			if ( !state.active || remaining <= 0.0f )
			{
				state.active = false;
				state.fade_alpha -= 3.5f * dt;
				if ( state.fade_alpha <= 0.0f )
				{
			...
			const bool is_warning = remaining <= 3.5f;
			const float alpha_scale = state.fade_alpha;

			xdraw::color base_col = cfg.color.value;
			if ( is_warning )
			{
				const float pulse = 0.5f + 0.5f * std::sinf( elapsed * 12.0f );
				base_col = xui::lerp( cfg.color.value, cfg.warning_color.value, pulse );
			}
			base_col.a = static_cast< std::uint8_t >( static_cast< float >( base_col.a ) * alpha_scale );

			// 1. 3D Ground Ring (radius 144.0f)
			if ( cfg.ground_ring.value )
			{
				constexpr int segs = 48;
				constexpr float two_pi = std::numbers::pi_v<float> * 2.0f;

				auto draw_circle_3d = [ & ]( float r, xdraw::color col, float thick )
				{
					for ( int i = 0; i < segs; ++i )
					{
						const float a0 = two_pi * static_cast< float >( i ) / static_cast< float >( segs );
						const float a1 = two_pi * static_cast< float >( ( i + 1 ) % segs ) / static_cast< float >( segs );

						auto p0 = state.origin + math::vector3{ std::cosf( a0 ) * r, std::sinf( a0 ) * r, 2.0f };
						auto p1 = state.origin + math::vector3{ std::cosf( a1 ) * r, std::sinf( a1 ) * r, 2.0f };
						...
						if ( w0 < 0.05f )
						{
							float t = std::clamp( ( 0.05f - w0 ) / ( w1 - w0 ), 0.0f, 1.0f );
							p0 = p0 + ( p1 - p0 ) * t;
						}
						else if ( w1 < 0.05f )
						{
						...
						const auto s0 = systems::g_view.project( p0 );
						const auto s1 = systems::g_view.project( p1 );

						if ( systems::g_view.projection_valid( s0 ) && systems::g_view.projection_valid( s1 ) )
						{
							draw_list.line( s0.x, s0.y, s1.x, s1.y, col, thick );
						}
					}
				};

				// Outer boundary ring (144.0u)
				draw_circle_3d( 144.0f, base_col, 2.0f );

				// Outer soft glow
				if ( cfg.glow.value )
				{
					auto glow_col = base_col;
					glow_col.a = static_cast< std::uint8_t >( static_cast< float >( glow_col.a ) * 0.4f );
					draw_circle_3d( 144.0f, glow_col, 4.5f );
				}

				// Inner expanding pulse echo
				const float pulse_r = 40.0f + std::fmodf( elapsed * 35.0f, 95.0f );
				auto pulse_col = base_col;
				pulse_col.a = static_cast< std::uint8_t >( static_cast< float >( pulse_col.a ) * 0.4f * ( 1.0f - ( ( pulse_r - 40.0f ) / 95.0f ) ) );
				draw_circle_3d( pulse_r, pulse_col, 1.5f );
			}

			// 2. 3D Floating Tactical Timer Billboard
			const auto billboard_origin = state.origin + math::vector3{ 0.0f, 0.0f, 50.0f };
			const auto billboard_proj = systems::g_view.project_full( billboard_origin );

			if ( billboard_proj.on_screen && billboard_proj.w > 0.05f )
			{
				const float bx = billboard_proj.screen.x;
				const float by = billboard_proj.screen.y;

				constexpr float w = 78.0f;
				constexpr float h = 26.0f;
				const float x = std::floorf( bx - w * 0.5f );
				const float y = std::floorf( by - h * 0.5f );

				// Dark badge background
				const auto bg_col = xdraw::color{ 12, 14, 20, static_cast< std::uint8_t >( 220.0f * alpha_scale ) };
				const auto border_col = is_warning ? cfg.warning_color.value : base_col;
				middle_draw_list.rect_filled( x, y, w, h, bg_col, xdraw::corner_radius{ 4.0f } );
				middle_draw_list.rect( x, y, w, h, border_col.alpha( static_cast< std::uint8_t >( 200.0f * alpha_scale ) ), xdraw::corner_radius{ 4.0f }, 1.2f );

				// Mini radial arc gauge
				constexpr float arc_r = 7.5f;
				const float arc_cx = x + 13.0f;
				const float arc_cy = y + h * 0.5f - 1.0f;

				middle_draw_list.circle( arc_cx, arc_cy, arc_r, xdraw::color{ 45, 50, 65, static_cast< std::uint8_t >( 180.0f * alpha_scale ) }, 1.5f, 20 );

				constexpr float half_pi = std::numbers::pi_v<float> * 0.5f;
				constexpr float two_pi = std::numbers::pi_v<float> * 2.0f;
				const float arc_len = frac * two_pi;
				const int arc_segs = std::max( 3, static_cast< int >( 20 * frac ) );

				std::vector<float> arc_pts;
				arc_pts.reserve( ( static_cast< std::size_t >( arc_segs ) + 1 ) * 2 );
				for ( int i = 0; i <= arc_segs; ++i )
				{
					const float t = static_cast< float >( i ) / static_cast< float >( arc_segs );
					const float a = -half_pi + t * arc_len;
					arc_pts.push_back( arc_cx + std::cosf( a ) * arc_r );
					arc_pts.push_back( arc_cy + std::sinf( a ) * arc_r );
				}
				if ( arc_pts.size( ) >= 4 )
				{
					middle_draw_list.polyline( std::span<const float>( arc_pts.data( ), arc_pts.size( ) ), border_col, false, 2.0f );
				}

				// Digital timer text
				char time_str[ 16 ]{};
				std::snprintf( time_str, sizeof( time_str ), "%.1fs", remaining );
				middle_draw_list.text( x + 26.0f, std::floorf( y + 6.0f ), time_str, xdraw::color{ 255, 255, 255, static_cast< std::uint8_t >( 245.0f * alpha_scale ) }, xdraw::text_style::shadowed );

				// Micro progress bar
				const float bar_w = ( w - 4.0f ) * frac;
				middle_draw_list.rect_filled( x + 2.0f, y + h - 3.0f, bar_w, 2.0f, border_col, xdraw::corner_radius{ 1.0f } );

				// Expiring alert
				if ( is_warning )
				{
					const auto [ww, wh] = xdraw::measure_text( "EXPIRING" );
					middle_draw_list.text( std::floorf( bx - ww * 0.5f ), std::floorf( y - wh - 2.0f ), "EXPIRING", cfg.warning_color.value, xdraw::text_style::outlined );
				}
			}
