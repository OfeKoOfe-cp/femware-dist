...
namespace systems {
	...
	public:
		...
		[[nodiscard]] static bool is_ignorez( std::uintptr_t mat );
		[[nodiscard]] static bool is_cham_material( std::uintptr_t mat );
	...
	public:
		...
		[[nodiscard]] math::vector3 get_view_angles( bool* out_ok = nullptr ) const;
	...
	public:
		...
		bool execute_buffer( const std::string& buffer, std::string& err_out );
		void on_render( xdraw::draw_list& draw_list );
		void add_render_callback( int ref );

		[[nodiscard]] bool is_initialized( ) const { return this->m_L != nullptr; }

	private:
		::lua_State* m_L{ nullptr };
		std::vector<int> m_render_callbacks{};
		std::unordered_map<int, int> m_render_error_counts{};
		std::mutex m_mutex{};
	};
